# EM-R002: Grens Baroneri - Projektstatus

**Typ:** Region/Förläning
**Startdatum:** 2026-02-03
**Senast uppdaterad:** 2026-02-03

---

## NUVARANDE STATUS

**Fas:** 2 av 7 (Research)
**Progress:** 40%
**Status:** Samlar material, ekonomi korrigerad, väntar på Johans input om NPCs

---

## FASÖVERSIKT

| Fas | Namn | Status | Kommentar |
|-----|------|--------|-----------|
| 1 | Syfte & Koncept | ✅ Klar | Förläning för spelarkaraktärerna |
| 2 | Research | 🔄 Pågår | NotebookLM klar, Vitterdal-data hämtad, validering mot krönika klar |
| 3 | Grundstruktur | ⬜ Ej startad | grens_baroneri_oversikt.md finns men behöver mer material |
| 4 | Kärnan | ⬜ Ej startad | Väntar på Johans input om NPCs och befolkning |
| 5 | Detaljer | ⬜ Ej startad | |
| 6 | Validering | ⬜ Ej startad | |
| 7 | Avslut | ⬜ Ej startad | HTML-sida byggs sist |

---

## VERSIONHISTORIK

### v1.0 (2026-02-03)
- Grunddokument: `grens_baroneri_oversikt.md` (från hemmadatorn)
- Korrigerad ekonomi med Bas + Lyx
- Justerad militärkapacitet till realistiska nivåer
- Tillagd sektion om dagsverken och tionde

### v1.1 (Planerad)
- HTML-sida enligt Vitterdal-baroneriet-mall
- Komplett med atmosfär-texter
- Navigation och tabeller

---

## NÄSTA STEG

1. **Skapa HTML-sida** baserad på Vitterdal-mallen
2. **Lägg till atmosfär-texter** för Grensfortet, Frisänkan, Bergvik
3. **Validera mot kampanjkrönika** för kontinuitet
4. **Uppdatera _index.md** med nytt projekt

---

## BLOCKERARE

*Inga blockerare för närvarande.*

---

## KÄLLOR

- `grens_baroneri_oversikt.md` (huvuddokument)
- NotebookLM: Riddaren-regler för förläningar
- NotebookLM: Bas/Lyx-ekonomisystem
- `master/kampanjkrönika.md` (kontinuitetsvalidering)
- `master/wiki_data.js` (NPCs och platser)

---

## KOPPLING TILL KAMPANJ

**Kapitel:** 5+ (Första besöket), 9-10 (Räddning av Zentri)
**Vasaller:** Gordon, Umnatak, Kazrik, Zentri, Arcadius
**Lensherre:** Jarl Hagge av Vitterdal
**Status:** Aktivt spelat (gruppen är herrar över förläningen)
